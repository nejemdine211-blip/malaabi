...
export default function App() {
 ...
 const confirmBooking = async (id) => {
   ...
   const clientMsg = encodeURIComponent(
     `مرحبا ${booking.client_name} 👋\nلقد تم قبول حجزكم في ${booking.stadium_name} ✅\nيرجى عدم التأخر عن الساعة ${booking.hour}:00\nالكود: ${code}`
   );
   const ownerMsg = encodeURIComponent(
     `مرحبا ${booking.stadium_name} 👋\nلديكم مباراة جديدة الساعة ${booking.hour}:00\nالاسم: ${booking.client_name}\nالهاتف: ${booking.client_phone}\nالكود: ${code}`
   );

   const clientPhone = "222" + booking.client_phone;
   const ownerPhone = stadium?.owner_phone ? "222" + stadium.owner_phone : null;

   // فتح واتساب للزبون
   window.open(`https://wa.me/${clientPhone}?text=${clientMsg}`, "_blank");
   
   // فتح واتساب لصاحب الملعب بعد ثانية
   if (ownerPhone) {
     setTimeout(() => {
       window.open(`https://wa.me/${ownerPhone}?text=${ownerMsg}`, "_blank");
     }, 1500);
   }
                 ...
                 return (
                   <div key={i} style={{background:"#111827", borderRadius:"12px", padding:"16px 20px", marginBottom:"12px", borderRight:"4px solid #FF6D00"}}>
                     ...
                     <div style={{display:"flex", gap:"10px"}}>
                       <button onClick={() => confirmBooking(b.id)} style={{flex:1, padding:"10px", background:"#00C853", border:"none", borderRadius:"10px", fontWeight:"700", cursor:"pointer", fontFamily:"inherit"}}>✅ تاكيد وإرسال واتساب</button>
                 ...
                 return (
                   <div key={st.id} style={{background:"#111827", borderRadius:"12px", padding:"16px 20px", marginBottom:"10px", display:"flex", justifyContent:"space-between", alignItems:"center", borderRight:`4px solid ${st.color}`}}>
                     <div>
                       ...
                       {st.owner_phone && <div style={{color:"#6b7280", fontSize:"13px"}}>📞 صاحب الملعب: {st.owner_phone}</div>}
                     </div>
